...
namespace ATuSalud.Controllers
{
    public class SqlQuery_4 : Controller
    {
        ...
        {
            string sql = "SELECT p.nombre, COUNT(1)/COUNT(DISTINCT DATE(c.Fecha_atencion)) FROM tablaprofesional p INNER JOIN tablacitaspaciente c ON(p.id= c.Id_profesional) GROUP BY p.id";
